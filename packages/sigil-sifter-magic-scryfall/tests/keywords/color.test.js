...
import ScryfallCardFace from '../../src';

beforeAll(() => {
    Magic(sifter, ScryfallCardFace);
});
...
describe('Color keyword', () => {
    ...
    test('negation works (-c:r -c:g -c:w -c:b excludes red, green, white, and black)', () => {
        const notRed = sifter.filter(cards, '-c=r -c:g -c:w -c:b');
        expectCardNames(notRed,  [
            'Solemn Simulacrum', 'Fell the Profane // Fell Mire',
            'Talisman of Creativity', 'Wonder', 'Snapcaster Mage', 'The One Ring',
            'The Kami War // O-Kagachi Made Manifest', 'Black Lotus', 'Sol Ring',
